-- pg-boss 10.4.2, schema version 24.
--
-- GENERATED, NOT HAND-WRITTEN. This is the output of
--
--   PgBoss.getConstructionPlans('pgboss')
--
-- with pg-boss's own BEGIN/COMMIT removed, because Prisma already runs each migration
-- file inside a transaction and a nested COMMIT would end Prisma's early. Regenerate it
-- the same way rather than editing it: pg-boss checks pgboss.version before it will
-- start, and a schema that differs from what this version of the library expects is a
-- worker that refuses to run.
--
-- Why a migration at all, rather than letting pg-boss migrate itself at start-up: the
-- release command runs migrations once, before the new version takes traffic, so the
-- queue exists before either process can try to use it. Self-migration would race two
-- machines and leave the API sending to a schema the worker had not created yet.
--
-- These tables are in the pgboss SCHEMA, not public. Prisma has no multiSchema setting,
-- so its differ never sees them and never proposes dropping them, and they are absent
-- from schema.prisma — which is why they need NO entry in platform/scope.ts. The scope
-- registry test reads schema.prisma, and an entry naming a model that is not there
-- fails its phantom check. They are unscoped by design and by construction: pg-boss
-- owns them, no application query reaches them, and the district a job belongs to
