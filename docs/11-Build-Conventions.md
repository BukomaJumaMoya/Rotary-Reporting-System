...
**`docs/schema.sql` is v2.1, not v1.0.** Amendments are logged in that file's header.
...
| `schema.sql` | v2.1 — `user_tokens.created_at`, the `session` table that was always missing, and in M2 `person_erasure_requests` plus the `club_rosters` correction fix |
